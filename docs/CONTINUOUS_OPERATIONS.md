...
2026-09-23追加: [時間外分析ガイド](EVENING_REPORTS.md) に翌営業日HTML/JSON、解析待ちの事前確認、拡張run-status、市場ニュース検索の修正を記載しています。以下の基本操作は引き続き使用できます。
...
- 日足は `refresh_yahoo` と `history_start` で制御。runtimeは16時JST以降に当日足も取得対象とします。それ以前と単独Yahoo CLIは当日足を除外します。通信エラー時に既存データは削除しません。正常取得時の銘柄ごとの更新は既存Yahoo保存処理と同じです。

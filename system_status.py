...
def snapshot(c,now):
    ...
    queue=preview(c,now)
    errors=[f"{name}: {j['status']}" for name,j in jobs.items() if j['status']=='failed' and (market or name not in ('prices','paper'))]
    warnings=[];stop=[]
    ...
    return dict(at=now.isoformat(),at_jst=local.isoformat(),calendar=calendar,
        bot_state='直近のハートビートあり（稼働の目安）' if active else '停止中または応答なし・起動未確認',heartbeat=heartbeat,
        market_state='取引時間内' if market else '市場時間外',
        last_success=success[0]['at'] if success else None,jobs=jobs,logs=logs,
        daily_prices=prices[0] if prices else dict(at=None,day=None),news_coverage=healthy,
...
def display(s):
    ...
    for name in ('news','prices','analysis','paper'):
        job=s['jobs'].get(name,{})
